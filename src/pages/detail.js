  ...
  return (
    <div className="px-4 mb-8 max-w-2xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-5">
        ...
        <div className="flex flex-col gap-2">
          <h1 className="text-2xl uppercase font-medium">{detail.name}</h1>
          <p className="font-semibold text-2xl">${detail.price}</p>
          <div className="mt-4">
            <span
              className={`py-1 px-3 text-base rounded-full font-medium ${detail.stockStatus === 'In Stock' ? 'bg-green-500 text-white' : 'bg-red-500 text-white'}`}
            >
              {detail.stockStatus}
            </span>
          </div>
          <div className="flex gap-5 items-center">
            <div className="flex gap-2 justify-center items-center">
              <button
                className="bg-gray-100 h-full w-10 font-bold text-xl rounded-lg flex justify-center items-center"
                onClick={handleMinusQuantity}
              >
                -
              </button>
              <span className="bg-gray-200 h-full w-10 font-bold text-xl rounded-lg flex justify-center items-center">
                {quantity}
              </span>
              <button
                className="bg-gray-100 h-full w-10 font-bold text-xl rounded-lg flex justify-center items-center"
            ...
            <button
              className="bg-slate-900 text-white px-7 py-3 rounded-lg shadow-lg"
      ...
      <div className="px-2 mb-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {products
            .filter((product) => product.id !== detail.id) // Exclude the current product
